...
RSpec.describe ReviewMailer, type: :mailer do
  ...
  it 'should not proceed if there is no event' do
    ...
    # @paper_assignment = Paper.new(tutor_id: 123, relator_id: 12, director_id: 32)


    # paper_assignment_hash = { tutor_id: 123, relator_id: 12, director_id: 32 }
    # paper_assignment = Paper.create(tutor_id: 123, relator_id: 12, director_id: 32)
    # ReviewMailer.review_notification("test@gmail.com", @paper_assignment, "paper", nil).deliver_now
    # expect(ActionMailer::Base.deliveries.count).to eq(0)
  end
